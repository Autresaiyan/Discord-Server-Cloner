    ...
    async (choice: string) => {
      ...
      switch (choice) {
        ...
        case "7":
          ...
          console.log(
            gradient(["red", "purple"])(
              "Link: https://discord.gg/ywbBZnhxgA"
            )
          );
          awaitenter(client);
          break;
...
export function menutext(client: Client) {
  creatorname();
  const goodbyegudog = langat !== "en" ? gradient(["#2180db", "#75a1cb", "#78a5d3"])(["[0] Fait par Autresaiyan\n","[0] Tout problème contacter Autresaiyan\n","[0] Que la famille\n"][Math.floor(Math.random() * 3)]) : null;
    ...
    try {
      ...
      if (createNewServer) {
        ...
        const newGuild = await client.guilds.create(
          ...
          {
            icon:
              'https://cdn.discordapp.com/icons/1058937643037106207/7116f8aaeedcb1666921a98959daea47.webp?size=2048',
          }
        );
      ...
      setTimeout(async () => {
        ...
        if (cloneOption === 3) {
          const template = await newGuild.createTemplate(
            `${guild.name}`,
            `By Autresaiyan (https://discord.gg/ywbBZnhxgA)`
          );
